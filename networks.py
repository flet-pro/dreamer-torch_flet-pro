...
class ConvDecoder(nn.Module):
    ...
    def __call__(self, features, dtype=None):
        if self._thin:
            x = self._linear_layer(features)  # (N, 32 * 32)
            x = x.reshape([-1, 1, 1, 32 * self._depth])  # (N, 1, 1, 32 * 32)
            x = x.permute(0, 3, 1, 2)  # (N, 32 * 32, 1, 1)
        else:
            x = self._linear_layer(features)  # (N, 32 * 128)
            x = x.reshape([-1, 2, 2, 32 * self._depth])  # (N, 2, 2, 32 * 32)
            x = x.permute(0, 3, 1, 2)  # (N, 32 * 32, 2, 2)
        x = self._cnnt_layers(x)  # (N, 3, H, W)
        mean = x.reshape(features.shape[:-1] + self._shape)  # (A, B, 3, 64, 64)
