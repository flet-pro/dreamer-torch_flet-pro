...
class WorldModel(nn.Module):
    ...
    def __init__(self, step, config):
        ...
        self.encoder = networks.ConvEncoder(config.grayscale,  # False
                                            config.cnn_depth, config.act, config.encoder_kernels)
        if config.size[0] == 64 and config.size[1] == 64:
            embed_size = 2 ** (len(config.encoder_kernels) - 1) * config.cnn_depth  # 2 ** 3 * 32 = 256
            embed_size *= 2 * 2  # 64 * 64 -> 2 * 2
        else:
    ...
    def _train(self, data):
        ...
        with tools.RequiresGrad(self):
            with torch.cuda.amp.autocast(self._use_amp):
                embed = self.encoder(data)  # select image inside encoder's forward

                post, prior = self.dynamics.observe(embed, data['action'])

                kl_balance = tools.schedule(self._config.kl_balance, self._step)  # always return 0.8
                kl_free = tools.schedule(self._config.kl_free, self._step)  # always return 1.0
                kl_scale = tools.schedule(self._config.kl_scale, self._step)  # always return 1.0 for dmc, 0.1

                kl_loss, kl_value = self.dynamics.kl_loss(
                    post, prior, self._config.kl_forward, kl_balance, kl_free, kl_scale)  # kl_forward: False

                losses = {}
                likes = {}
                for name, head in self.heads.items():
                    grad_head = (name in self._config.grad_heads)  # bool

                    feat = self.dynamics.get_feat(post)  # common feature from rssm
                    feat = feat if grad_head else feat.detach()
                    pred = head(feat)  # feat into each head
    ...
    def preprocess(self, obs):
        ...
        elif self._config.clip_rewards == 'identity':  # default and dmc
            obs['reward'] = torch.Tensor(obs['reward']).unsqueeze(-1)
        else:
            raise NotImplemented(f'{self._config.clip_rewards} is not implemented')

        if 'discount' in obs:
            obs['discount'] *= self._config.discount  # * 0.99
