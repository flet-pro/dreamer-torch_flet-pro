...
def simulate(agent, envs, steps=0, episodes=0, state=None):
    ...
    if state is None:
        ...
        step, episode, done, length, obs, agent_state, reward = state  # note state include all these!?
...
def static_scan(fn, inputs, start):
    last = start
    indices = range(inputs[0].shape[0])  # batch_length
    flag = True
    for index in indices:
        inp = lambda x: (_input[x] for _input in inputs)
        last = fn(last, *inp(index))  # index's action and embed of batch_length dim
        if flag:
            if type(last) == type({}):
