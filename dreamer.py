...
def main(config):
    ...
    # todo what can load episode and saved logs can do in restart?????
    eval_eps = tools.load_episodes(directory, limit=1)

    # todo 1. read the code of make env function
    make = lambda mode: make_env(config, logger, mode, train_eps, eval_eps)

    train_envs = [make('train') for _ in range(config.envs)]
    eval_envs = [make('eval') for _ in range(config.envs)]
    acts = train_envs[0].action_space
    # todo what differ between dmc, atari and dmlab????
...
if __name__ == '__main__':
    ...
    parser.add_argument('--configs', nargs='+', required=True)  # 1つ以上の引数をリストで受け取る
