...
os.environ['MUJOCO_GL'] = 'glfw'  # MacOS (egl)
...
def main(config):
    ...
    # todo 午前中にここまで
    # makeの部分のwrapper　＋　simulate agent等の意味（以下の部分）
    # 具体的な流れがつかめて，画像等のデータのやり取りをつかめればok?
    # networkの定義部分のネストに注意

    if not config.offline_traindir:
        prefill = max(0, config.prefill - count_steps(config.traindir))
        print(f'Prefill dataset ({prefill} steps).')
        if hasattr(acts, 'discrete'):  # todo discreteを持っている？
